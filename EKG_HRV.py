...
#Some pre-calculated  parameters
    ...
    hrv_name = ['nni_mean', 'hr', 'hr_std', 'nni_diff', 'sdnn', 'rmssd', 'sdsd', 'nn50', 'pnn50', 'nn20', 'pnn20', 'tinn','tri_index', 'fft_ratio', 'fft_total','lomb_ratio', 'lomb_total', 'ar_ratio', 'ar_total', 'sd1', 'sd2', 'sd_ratio']
    ...
    def processData (signal, name):
        minuteS = []
        for i in range(0, len(signal), 15000):
            minuteS.append(signal[i:i+15000])

        if len(minuteS)>=4:
            minuteS = minuteS[-5:-1:]

        print(name, len(minuteS))

        filterS = []
        b, a = scipy.signal.butter(N, [2 / 125, 40 / 125], 'band')
        for i in range(0, len(minuteS)):
            f = scipy.signal.filtfilt(b, a, minuteS[i])
            filterS.append(f)
        ...
        for i in range(0, len(invS)):
            pikovi, _ = scipy.signal.find_peaks(invS[i], height=55, distance=50, prominence=50)
            pik = pikovi[0]
            niz_pikova = [pik]
            ...
            for x in pikovi[1:]:
                if x - pik >= P:
                    niz_pikova.append(x)
                    pik = x
            nizPIK.append(niz_pikova)


        peaks_x = nizPIK
        peaks_y = []
        for i in range(0, len(filterS)):
            peak = filterS[i][nizPIK[i]]
            peaks_y.append(peak)
        ...
        for i in range(0, len(peaks_x)):
            for znj in range(100):
                for x in peaks_x[i][0:]:
                    if peaks_y[i][peaks_x[i].index(x)] < filterS[i][x - 1]:
                        peaks_y[i][peaks_x[i].index(x)] = filterS[i][x - 1]
        ...
        return filterS, peaks_x, peaks_y, HRV


    def processHRV (HRV, index):
        hrv_param = []
    ...
    filterN, peak_xN, peak_yN, hrvN = processData(neutral, 'neutral')
    filterS, peak_xS, peak_yS, hrvS = processData(stress, 'stress')
    filterC, peak_xC, peak_yC, hrvC = processData(calm, 'calm')
    ...
    hrv_paramN = processHRV(hrvN, hrv_index)
    hrv_paramS = processHRV(hrvS, hrv_index)
    hrv_paramC = processHRV(hrvC, hrv_index)
